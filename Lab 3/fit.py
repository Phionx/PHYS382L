...
for i in range(num_files):
	...
	times.append(sigma_X_i)
	times.append(sigma_Y_i)

times     = np.array(times).astype(np.float)
sigma_X   = np.array(sigma_X).astype(np.float)
sigma_Y   = np.array(sigma_Y).astype(np.float)
...
def pi_pulse_func(x, xerr, *parameters):
	t      = x
	tau    = parameters[0]
	delta  = parameters[1] 
	t_0    = parameters[2]
...
for iteration in range(num_files):
	...
	var                       = [[-100000, -100000] for x in range(len(parameter_estimates))]
	...
	start_index               = find_nearest(xdata_data, .005)
	end_index                 = find_nearest(xdata_data, time_pulses-.0001)
	...
	title    = "$\\sigma_x$ vs Time for $t_{sweep} = time_pulse$ (s)"
	y_label  = "$\\sigma_x$ (V)"
	x_label  = "Time $t$ (s)"
...
sigma_X_peaks_times.append(Optimal_params[2])
sigma_X_peaks_times_err.append(Optimal_params_err[2])
sigma_X_peaks.append(pi_pulse_func(Optimal_params[2], *Optimal_params)) #(t_0, sigma_x(t_0))
sigma_X_peaks_err.append()
	

	sigma_X_peaks_times.append(Optimal_params[0])
	sigma_X_peaks.append()
