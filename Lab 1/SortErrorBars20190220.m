...
date_written       = '20190220'; %Date of Data Analysis
...
file_reading              = strcat('Data/measurementsAnalysis/LamoreauxCombinedErrorBars/', date_acquired, '_', junction_type, '_', tunneling_type, '_', 'Trial', string(trial), '.csv');
disp(file_reading);
data                      = csvread(file_reading);
data                      = sortrows(data); %sorts by first row values, this is to get our Input_V_j in order!
file_writing              = strcat('Data/measurementsAnalysis/SortedErrorBars/', date_written , '_', junction_type, '_', tunneling_type, '_', 'Trial', string(trial), '.csv');
